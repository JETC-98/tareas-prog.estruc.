...
int main() {
	...
	for (i=1;i<=clientes;i++){
		...
		for(x=1;x<=pedido;x++){
			...
			if(orden==ordenu){
				cs++;
			}else if(orden==ordend){
				cd++;
			}else if(orden==ordent){
				ct++;
			}
			
		}
		total1=((cs*H1)+(cd*H2)+(ct*H3));
		cs=0;
		cd=0;
		ct=0;
...
/*Lo mismo que el sin #define,la variable "totalventas" no se usa y al usar la opcion
de tarjeta de credito me da valores por arriba de los 100k a pesar de que solo estoy pidiendo
una hamburguesa*/
//Josue Torres
