...
negativos, cuantos son positivos y cuantos fueron iguales a cero.Nombre:48
...
int main(int argc, char *argv[]) {
	...
	for(int i=0; i<CEmpleados; i++){
		...
		if(Num <cero){//(Num <0)
			Neg = (Neg +1);
		}else if(Num>cero){//(Num>cero)
			Pos=(Pos+1);
		}else if(Num==cero){//(Num==cero)
			Cer=(Cer+1);
		}
		
		//para que utilizar literales cuando pudieron haber puesto numeros, entiendo el uso de define, pero no era necesario para 0
		system("cls");
	}
	...
	//fuera del define 0 que se me hace innecesario el programa funciona bien.
	//Daniel Encalada
